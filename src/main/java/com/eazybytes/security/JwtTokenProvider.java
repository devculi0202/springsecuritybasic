...
public class JwtTokenProvider {
  ...
  public String createToken(String userId, String email) {
    Claims claims = Jwts.claims().setSubject(email).setAudience(userId);
    Date now = new Date();
    Date expiryTime = new Date(now.getTime() + expiryTimeMilliseconds);
    return Jwts.builder().setId(UUID.randomUUID().toString()).setIssuedAt(now).setIssuer(email)
  ...
  public String resolveToken(String token) {
    try {
      ...
      throw new UnauthorizedException("Expired or invalid JWT token", HttpStatus.UNAUTHORIZED);
    }
  }
}
